:- dynamic perception/5,executedAction/2,position/3,comida/3,enemigo/3,vacia/3,
...
%% Funcion probada.
%posicion(X,Y,1):-perception([_,_,_,_],X,Y,_,1),!.

%position(1,1,1):-perception([_,_,_,_,_],1).

stench(X,Y,S):-perception([stench,_,_,_,_],S),position(X,Y,S).
breeze(X,Y,S):-perception([_,breeze,_,_,_],S),position(X,Y,S).
glitter(X,Y,S):-perception([_,_,glitter,_,_],S),position(X,Y,S).
bump(X,Y,S):-perception([_,_,_,bump,_],S),position(X,Y,S).
wumpusScream(X,Y,S):-perception([_,_,_,_,wumpusScream],S),position(X,Y,S).

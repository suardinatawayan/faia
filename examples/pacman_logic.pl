...
actions([],0):-!.
actions([A|As],S):-executedAction(A,S),S1 is S-1,actions(As,S1).
moveAction(S):-executedAction(goup,S).
moveAction(S):-executedAction(godown,S).
moveAction(S):-executedAction(goright,S).
moveAction(S):-executedAction(goleft,S).
...
% Mirar cómo está definida 'adjancent'. Esto se traduce a: es muy bueno
% moverse hacia una celda adyacente donde hay comida o un enemigo.
very_good(D,S):-adjancent(Xa,Ya,D,S),food(Xa,Ya,S).
very_good(D,S):-adjancent(Xa,Ya,D,S),enemy(Xa,Ya,S).
...
% adyacente vacía.
regular(D,S):-adjancent(Xa,Ya,D,S),empty(Xa,Ya,S).
...
adjancent(X,Y1,goleft,S):- position(X,Y,S),addPosition(Y,-1,Y1).
adjancent(X,Y1,goright,S)  :- position(X,Y,S),addPosition(Y,1,Y1).
adjancent(X1,Y,goup,S)   :- position(X,Y,S),addPosition(X,-1,X1).
adjancent(X1,Y,godown,S)    :- position(X,Y,S),addPosition(X,1,X1).

discoveryDirection(D,S):-adjancent(Xa,Ya,D,S),adjacentCell(Xa,Ya,Xaa,Yaa),not(knows(Xaa,Yaa,S)).
foodDirection(D,S):-adjancent(Xa,Ya,D,S),adjacentCell(Xa,Ya,Xaa,Yaa),food(Xaa,Yaa,S).
enemyDirection(D,S):-adjancent(Xa,Ya,D,S),adjacentCell(Xa,Ya,Xaa,Yaa),enemy(Xaa,Yaa,S).
...
% movingEnergyData([],1):-!.
% movingEnergyData([D|Ds],S1):-S0 is S1-1,moveAction(S0),energy(E0,S0),energy(E1,S1),D is E1-E0,movingEnergyData(Ds,S0),!.

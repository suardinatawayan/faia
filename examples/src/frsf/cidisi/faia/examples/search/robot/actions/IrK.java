...
public class IrK extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (estR.getPosicionesVisitadas().contains(EstadoRobot.K)) {
            return null;
        }
        ArrayList<String> sucesores = new ArrayList<String>(estR.getSucesores());
        if (sucesores != null) {
            int index = sucesores.indexOf(EstadoRobot.K);
            if (index >= 0) {
                estR.setPosicion(EstadoRobot.K);
                return estR;
            }

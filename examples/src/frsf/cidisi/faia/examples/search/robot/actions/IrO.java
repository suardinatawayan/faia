...
public class IrO extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (estR.getPosicionesVisitadas().contains(EstadoRobot.O)) {
            return null;
        }
        ArrayList<String> sucesores = new ArrayList<String>(estR.getSucesores());
        if (sucesores != null) {
            int index = sucesores.indexOf(EstadoRobot.O);
            if (index >= 0) {
                estR.setPosicion(EstadoRobot.O);
                return estR;
            }

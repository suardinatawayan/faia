...
public class IrN extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (estR.getPosicionesVisitadas().contains(EstadoRobot.N)) {
            return null;
        }
        ArrayList<String> sucesores = new ArrayList<String>(estR.getSucesores());
        if (sucesores != null) {
            int index = sucesores.indexOf(EstadoRobot.N);
            if (index >= 0) {
                estR.setPosicion(EstadoRobot.N);
                return estR;
            }

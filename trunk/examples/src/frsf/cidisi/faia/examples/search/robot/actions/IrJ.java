...
public class IrJ extends SearchAction {
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s) {
        ...
        if (estR.getPosicionesVisitadas().contains(EstadoRobot.J)) {
            return null;
        }
        ArrayList<String> sucesores = new ArrayList<String>(estR.getSucesores());
        if (sucesores != null) {
            int index = sucesores.indexOf(EstadoRobot.J);
            if (index >= 0) {
                estR.setPosicion(EstadoRobot.J);
                return estR;
            }

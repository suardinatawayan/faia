...
public class IrP extends SearchAction {
    ...
    public AgentState execute(AgentState s) {
        ...
        if (estR.getPosicionesVisitadas().contains(EstadoRobot.P)) {
            return null;
        }
        ArrayList<String> sucesores = new ArrayList<String>(estR.getSucesores());
        if (sucesores != null) {
            int index = sucesores.indexOf(EstadoRobot.P);
            if (index >= 0) {
                estR.setPosicion(EstadoRobot.P);
                return estR;
            }

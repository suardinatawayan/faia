...
public class IrE extends Action {
	...
	public AgentState execute(AgentState s){
		...
		if (estR.getPosicionesVisitadas().contains(EstadoRobot.E))
			return null;
		
		ArrayList<String> sucesores = new ArrayList<String>(estR.getSucesores());
		if (sucesores!=null){
			int index = sucesores.indexOf(EstadoRobot.E);
			if (index>=0){
				estR.setPosicion(EstadoRobot.E);
				return estR;
			}
